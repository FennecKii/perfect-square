...
func play_background_music(music_track: AudioStream, volume: float = 0.0, pitch: float = 0.0) -> void:
	...
	background_music.stream.loop_begin = 0 * music_track.sound.mix_rate
	background_music.stream.loop_end = music_track.sound.get_length() * music_track.sound.mix_rate
	background_music.play()

func play_sfx(sfx_stream: AudioStream, volume: float = 0.0, pitch: float = 0.0):
